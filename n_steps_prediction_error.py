...
for idx, dataset in enumerate(dataset_list):
    ...
    while i < len(X_train)-1:
        ...
        for j in range(n_steps):
            if i+j >= len(X_train)-1:
                break
            if len(mae_per_step_list)>=49:
                break
            #print(f'Predictor: {X_train[i+j,0]}')

            x_next = doc.step(A, B, x_next, U_train[i+j])
            #print(f'Target: {X_train[i+j+1,0]}')
            #print(f'Predicted: {x_next[0]}')
            #print()

            # Compute the MAE per time step (NaN values will be ignored)
            mae_per_step = np.mean(np.abs(x_next - X_train[i+j+1]))
            mae_per_step_list.append(mae_per_step)
    
            # Get track of predicion deterioration over n_steps
            mae_over_n_step_list.append(mae_per_step)

            if np.isnan(X_train[i+j+1]).any():
                continue
            predicted_states.append(x_next)
            real_predictor_states.append(X_train[i+j])
            real_target_states.append(X_train[i+j+1])
        if len(mae_over_n_step_list) == n_steps:
            mae_n_steps_overall.append(mae_over_n_step_list)
        i += n_steps
    """
    ...
    while i < len(X_test)-1:
        ...
        if np.isnan(X_test[i]).any() or np.isnan(X_test[i + 1]).any():
                i += 1
                continue
        X_train = X[:split_index+i]
        U_train = U[:split_index+i]
        A , B, lmbda = utils.stable_ridge_regression(X_train, U_train)
        x_next = X_test[i]
        for j in range(n_steps):
            if i+j >= len(X_test)-1:
                break
            x_next = doc.step(A, B, x_next, U_test[i+j])

            # Compute the MAE per time step nan values
            mae_per_step = np.mean(np.abs(x_next - X_test[i+j+1]))
            mae_per_step_list.append(mae_per_step)

            # Get track of predicion deterioration over n_steps
            mae_over_n_step_list.append(mae_per_step)
            
            if np.isnan(X_test[i+j+1]).any():
                continue
            predicted_states.append(x_next)
            real_predictor_states.append(X_test[i+j])
            real_target_states.append(X_test[i+j+1])
        if len(mae_over_n_step_list) == n_steps:
            mae_n_steps_overall.append(mae_over_n_step_list)
        i += n_steps
...
mae_n_steps_overall_array = np.array(mae_n_steps_overall)
mae_n_steps_overall_mean = np.nanmean(mae_n_steps_overall_array, axis = 0)

print(f'Shape of lists : {mae_n_steps_overall_array.shape}')
...
plt.figure()
plt.plot(mae_n_steps_overall_mean, linestyle='-', color='blue', label='MAE for n steps')

# Add titles and labels for the second plot
plt.title("MAE for 10 Steps (Across datasets with >= 70 valid rows, on test data)")
plt.xlabel("Step")
plt.ylabel("MAE")
plt.legend()  # Add legend for the second plot
