...
# Mean imputation method for handling missing data
...
for dataset, file in zip(dataset_list, files):
    ...
    for split in splits:
        ...
        split_index = np.searchsorted(np.cumsum(valid_rows), total * split) # searches for the index in (np.cumsum(pairs)) where the cumulative sum first exceeds or equals 70
