...
def bootstrap_ci(data, func=np.mean, n_bootstrap=10000, ci=95, seed=None):
    """
    Computes a bootstrap confidence interval for a given statistic (here: mean deviation of observations preceding missing EMA data).
    Returns: (lower bound, upper bound) of the confidence interval (confidence level 95%).
    """
    if seed is not None:
        np.random.seed(seed)
