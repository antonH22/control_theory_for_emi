...
def get_training_set(ratio, X_train, U_train):
    # Returns the training set with lower ratio
    target_ratio = ratio - 0.1
    ...
    X_last, U_last, ratio_last = X_train, U_train, ratio
...
for idx, dataset in enumerate(dataset_list):
    ...
    while current_ratio > 0.1:
        ...
        for i in range(num_resubsampling):
            X_train_ratio, U_train_ratio, ratio_last = get_training_set(current_ratio, X_train, U_train)
            if current_ratio == ratio_last or ratio_last == 0.0:
                skip = True
                continue
            current_ratio, X_train, U_train = ratio_last, X_train_ratio, U_train_ratio
