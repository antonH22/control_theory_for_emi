...
for subfolder in subfolders:
    ...
    for file in glob.glob(folder_path):
        ...
        files.append(file)
    for file_s in glob.glob(folder_path_smoothed):
...
for idx, dataset in enumerate(dataset_list):
    ...
    X_s, U_s = dataset_list_smoothed[idx]['X'], dataset_list_smoothed[idx]['Inp']
    ...
    for i in range(len(X_test) -1):
        ...
        X_train = np.append(X_train, X_train2[i:i+1], axis=0)
        U_train = np.append(U_train, U_train2[i:i+1], axis=0)
